...
**Arquivo de bugs:** `skills/qa-vplight/bugs.md` — é lá que ficam todos os bugs. O SKILL.md não registra bugs.
...
| Protocolo | Art-Net UDP porta 6454 | Saída DMX para SL3000 → XLR → fixtures |
...
| Engine | engine.js | Loop 40ms: interpolator → renderFrame → sendArtDMX → onFrame |
...
electron/
  ...
  engine/
    engine.js       → setInterval 40ms: interpolator + compositor.renderFrame() + sendArtDMX()
    ...
    interpolator.js → speed virtual para moving heads

src/
  store/showStore.js → ShowProvider, useShow(), updateScene, toggleScene
  screens/Main.jsx   → resolveUniverseState, handleBlackout, handleToggleArtNetFreeze
  screens/FixturePanel.jsx
  screens/FixtureEditor.jsx
  screens/PainelOperacao.jsx
  screens/Viewer3D.jsx
...
Operador clica (renderer)
  └─ window.vp.*()          [preload bridge]
       └─ ipcMain.handle()  [main.js]
            ├─ universe.setChannel()
            ├─ compositor.setSceneLock() → _sceneLock
            └─ show.updateScene()

engine loop 40ms
  └─ interpolator.tick()
  └─ compositor.renderFrame()
       ├─ OnExecute() por camada ativa
       └─ merge HTP/linear + guards
  └─ sendArtDMX(getUniverse())  ← bloqueado se artnet frozen
  └─ engine.onFrame → viewer 3D (IPC)
```
...
1. **Canal não chega ao universo?** → `dmx:getUniverse` (polling)
2. **Script não roda?** → `script:getAll` com `running: true`?
3. **Script roda mas canal não muda?** → `_sceneLock` / cena ativa nos mesmos canais
4. **Cena não salva?** → `vp.show.json` → `pages[p].scenes[key].channels`
5. **Palco não congela?** → `artnet:setFrozen` / início de `sendArtDMX`
6. **Fixture ignorado?** → `enabled: false`
