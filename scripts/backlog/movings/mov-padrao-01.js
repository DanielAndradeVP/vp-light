...
// ── Estado ──────────────────────────────────────────────────────────────────
let tick = 0;
let phase = 'reposition'; // 'reposition' (snap rápido, sem luz) | 'descend' (loop visível)
let descendTick = 0;
...
function OnExecute() {
  ...
  if (phase === 'reposition') {
    ...
    const p = clamp01(descendTick / (DESCEND_TICKS - 1));
    ch(m1_tilt, lerp(MP_M1.TILT_F, MP_M1.TILT_A, p));
    ch(m2_tilt, lerp(MP_M2.TILT_F, MP_M2.TILT_A, p));
    fechoOn = true;

    descendTick++;
    if (descendTick >= DESCEND_TICKS) {
      phase = 'reposition';
    }
  }

  ch(m1_fecho, fechoOn ? 255 : 0);
  ...
  ch(b02, spulse(tick + 25, 76, 255, 100));
  ch(b03, spulse(tick + 50, 76, 255, 100));
  ch(b04, spulse(tick + 75, 76, 255, 100));
