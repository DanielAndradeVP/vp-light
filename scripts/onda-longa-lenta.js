// onda-longa-lenta — Varredura horizontal ampla dos dois movings (mesmo princípio de
// curso amplo do mov-traj-rib-baixo), só que BEM lenta — ciclo de vários segundos por
// fase. Ribalta acompanhando com tilt lento, fita em pulso lento.
...
// Curso amplo — extremos reais do pan/tilt, mesmo princípio de mov-traj-rib-baixo.
const MH_X = { PAN_L: 0, PAN_R: 255, TILT_UP: 0, TILT_DN: 255 };

const SWEEP_PERIOD = 650;   // ~26s ciclo completo (~13s por fase) — bem lento
const MH_SPEED_RAW = 225;   // virtual_speed alto = lento

const FITA_PULSE_PERIOD = 260; // pulso lento independente da varredura
const FITA_MIN = 60;
const FITA_MAX = 255;
...
function OnStart() {
  tick = 119; // fase inicial deslocada — ribalta/varredura já começam perto do extremo alto em
...
function OnExecute() {
  ...
  // Movings — varredura ampla, bem lenta, MH1 e MH2 sempre juntos (mesmo t, gap fixo).
  ...
  ch(mh1Strobo, 255); // aberto/sem estrobo
  ch(mh2Strobo, 255);
  ...
  // Ribalta em par — tilt acompanha o mesmo progresso da varredura, bem devagar.
  ...
  // Fita — pulso lento, independente da varredura.
  ch(fita, lerp(FITA_MIN, FITA_MAX, wave01(tick, FITA_PULSE_PERIOD)));
}
