...
### 1. Triagem e Filtro Inicial (Diretório Bateamento)
* **Diretório:** `docs/auditorias/bateamento/`
* **Conceito:** Esta pasta recebe o contexto geral bruto do sistema. Este diretório retém o que é essencial. É a partir da análise e filtragem dos dados concentrados aqui que nasce o fluxo para criar uma nova `mina`.

### 2. Definição do Escopo Bruto (Exploração e Mina)
* **Diretório:** `docs/auditorias/exploracao/[nome-da-mina]/`
* **Regra:** O nível `exploracao` padroniza a fase de descoberta técnica pós-bateamento. A variável `mina` representa o escopo bruto extraído e isolado para análise profunda. Este identificador deve estar predefinido antes de qualquer criação.

### 3. Auditorias de Grande Porte (Pepita Bamburro)
...
### 4. Auditorias de Médio Porte (Pepitas)
...
⚠️ **Diretriz de Execução:** Antes de iniciar a escrita, valide se o caminho completo até a `[nome-da-mina]` dentro de `exploracao/` já foi gerado a partir do processo de `bateamento`. Toda `pepita-bamburro` criada deve estar obrigatoriamente vinculada a este escopo bruto predefinido.

* **Regra de execução da auditoria:** o auditor principal deve atuar como a **mente da auditoria**, responsável por raciocinar, decidir o foco, cruzar evidências, identificar riscos e concluir os problemas. O **codex-xhigh** deve atuar como o agente executor/de apoio (**braços e pernas**), fazendo a parte trabalhosa: ler arquivos, buscar trechos, coletar evidências, mapear fluxos e devolver achados organizados para o auditor principal analisar.
